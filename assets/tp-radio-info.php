<?php
	...
	function getIcecastInfo($streamUrl) {
		...
		$pathSegments[count($pathSegments) - 1] = 'status-json.xsl';
		$newPath = implode('/', $pathSegments);
		$statusUrl = isset($urlParts["port"]) ? "{$urlParts['scheme']}://{$urlParts['host']}:{$urlParts['port']}{$newPath}" : "{$urlParts['scheme']}://{$urlParts['host']}{$newPath}";
