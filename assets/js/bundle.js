...
var titleAnimationInterval = null;
...
class tPlayerClass {
	constructor(options) {
		...
		this.playerState = {
			_audioEvent: null,
			_autoplay: false,
			_playlistScrollbarHideDelay: null,
			_repeat: false,
			_shuffle: false,
			_status: null,
			_volumeToggle: false,
			...
			set audioEvent(newValue) {
				if(this._audioEvent !== newValue) {
					this._audioEvent = newValue;
			...
			set status(newValue) {
				if(this._status !== newValue) {
					...
					this.handlePlayerStatusChange(`Player Status: ${this._status}`);
				}
			},
		
			handlePlayerStatusChange: (state) => {
			...
			set isPlaylist(newValue) {
				this._isPlaylist = newValue;
			},
		
			get isRadioInfoUpdateAllowed() {
				return this._isRadioInfoUpdateAllowed;
			},
		
			set isRadioInfoUpdateAllowed(newValue) {
				this._isRadioInfoUpdateAllowed = newValue;
			},
		
			get isUserAdjustingVolume() {
				return this._isUserAdjustingVolume;
			},
		
			set isUserAdjustingVolume(newValue) {
				this._isUserAdjustingVolume = newValue;
			},
		
			get isUserSeekingAudio() {
				return this._isUserSeekingAudio;
			},
		
			set isUserSeekingAudio(newValue) {
				this._isUserSeekingAudio = newValue;
			},
		
			get isVolumeMuted() {
				return this._isVolumeMuted;
			},
		
			set isVolumeMuted(newValue) {
				this._isVolumeMuted = newValue;
			}
		}
	...
	async createPlayerInterface() {
		return new Promise((resolve, reject) => {
			try {
				...
				buttonIcons = rounded ? buttonIcons.rounded : buttonIcons.default;
				...
				this.createButtonIcon(this.uiElements.playbackButton, [], [buttonIcons.playback.play]);
				...
				const buttonsConfig = [
					{ condition: isPlaylist, classNames: ['tp-prev-button', 'tp-button'], icon: buttonIcons.prev, key: 'prevButton' },
					{ condition: showRepeatButton, classNames: ['tp-repeat-button', 'tp-button'], icon: buttonIcons.repeat, key: 'repeatButton' },
					{ condition: isPlaylist, classNames: ['tp-next-button', 'tp-button'], icon: buttonIcons.next, key: 'nextButton' },
					{ condition: isPlaylist && showShuffleButton, classNames: ['tp-shuffle-button', 'tp-button'], icon: buttonIcons.shuffle, key: 'shuffleButton' },
					{ condition: showShareButton, classNames: ['tp-share-button', 'tp-button'], icon: buttonIcons.share.closed, key: 'shareButton' }
				];
				...
				if(isPlaylist) {
					...
					this.createButtonIcon(this.uiElements.togglePlaylistButton, ['tp-stroke'], [buttonIcons.playlist.closed])
				} else {
				...
				if(!isMobile) {
					...
					this.createButtonIcon(
						...
						[buttonIcons.volume.speaker, buttonIcons.volume.line_1, buttonIcons.volume.line_2, buttonIcons.volume.muted]
					);
				...
				if(this.settings.showShareButton) {
					...
					this.createButtonIcon(this.uiElements.facebookButton, ['tp-fill'], [buttonIcons.facebook]);
					this.uiElements.twitterButton = createElement('button', ['tp-twitter-button', 'tp-button'], soicalWrapper);
					this.createButtonIcon(this.uiElements.twitterButton, ['tp-fill'], [buttonIcons.twitter]);
					this.uiElements.tumblrButton = createElement('button', ['tp-tumblr-button', 'tp-button'], soicalWrapper);
					this.createButtonIcon(this.uiElements.tumblrButton, ['tp-fill'], [buttonIcons.tumblr]);
				}
				...
				this.createButtonIcon(this.uiElements.errorClose, ['tp-stroke'], [buttonIcons.playlist.closed]);
	...
	createLink(type, href, parent) {
		...
		this.createButtonIcon(link, ['tp-stroke'], [buttonIcons[type]]);
	}
	...
	pause() { 
		...
		playbackButton.querySelector('path').setAttribute('d', buttonIcons.playback.play); 
		// Disable radio info updates 
		this.playerState.isRadioInfoUpdateAllowed = false; 
	} 
	...
	play() { 
		...
		playbackButton.querySelector('path').setAttribute('d', buttonIcons.playback.pause); 
		// Set the seeking state to true 
		this.isSeeking(true); 
